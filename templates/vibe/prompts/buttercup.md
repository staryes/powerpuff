...
Implement Blossom's verification items as simple tests in this run's test area, run them, inspect the results, and write `approved` / `changes-requested` / `blocked` with the test results into your handoff (Kotodute format, see `powerpuff/kotodute.md`); validate it with `python3 powerpuff/scripts/koto-check.py` before returning. Return a one-paragraph status to Misato.
...
Ask-tier operations may be attempted normally - the harness prompt to the human is the approval. If you hit a human-only (deny tier) operation, add a PENDING TODO to `powerpuff/human-todo.md`, note it in your handoff, and return.
