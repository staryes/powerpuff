...
printf '%s' "$cmd" | grep -qE '^python3 powerpuff/scripts/koto-check\.py [^;&|<>]*$' && exit 0
...
for p in 'powerpuff/task/scope\.md' 'powerpuff/human-todo\.md' 'powerpuff/scripts/' '\.claude/' '\.opencode/' '\.vibe/' '\.pi/'; do
